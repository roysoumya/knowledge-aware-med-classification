...
def cadec_convert_examples_to_features(
    examples,
    tokenizer,
    max_length=512,
    task=None,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if output_mode == "multiclassification":
            label = example.label
        elif output_mode == "regression":
            label = float(example.label)
        else:
...
class ICHIProcessor(DataProcessor):
    """Processor for the MultiNLI data set (GLUE version)."""
...
class CADECProcessor(DataProcessor):
    """Processor for the MultiNLI data set (GLUE version)."""
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
            guid = "%s-%s" % (set_type, i)
            label = [int(x) for x in line[3:8]]
            heading = None
            text = line[2]
            examples.append(
                InputExample(guid=guid, text_a=text, text_b=heading, label=label))
        return examples
