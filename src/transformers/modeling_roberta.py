...
ROBERTA_PRETRAINED_MODEL_ARCHIVE_MAP = {
    ...
    "roberta-large": "/home/bt1/17CS10037/new_transformers/transformers/roberta-large-pytorch_model.bin",
...
class RobertaForSequencemultiClassification(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels]``.
    ...
    def __init__(self, config):
        super(RobertaForSequencemultiClassification, self).__init__(config)
        self.num_labels = config.num_labels
