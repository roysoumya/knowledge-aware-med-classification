...
class PreTrainedTokenizer(object):
    ...
    def num_added_tokens_lcf(self, pair=False, n_aspects=0):
        """
        Returns the number of added tokens when encoding a sequence with special tokens.
        ...
        for i in range(n_aspects):
            aspects_ids.append([])
        return len(self.build_inputs_with_special_tokens_lcf(token_ids_0, token_ids_1 if pair else None, aspects_ids if n_aspects!=0 else None))
        ...
        **kwargs
        ...
        if aspects is not None:
            ...
            for aspect in aspects:
                aspects_ids.append(get_input_ids(aspect))
        else:
            aspects_ids = None

        return self.prepare_for_model_lcf(
            first_ids,
            pair_ids=second_ids,
            aspects_ids=aspects_ids,
            max_length=max_length,
            pad_to_max_length=pad_to_max_length,
            add_special_tokens=add_special_tokens,
            stride=stride,
    ...
    def prepare_for_model_lcf(
        ...
        len_ids = len(ids)
        len_pair_ids = len(pair_ids) if pair else 0
        aspect_present = bool(aspects_ids is not None)
        len_aspects_ids = 0 
        if aspect_present:
            n_aspects = len(aspects_ids)
            for aspect_ids in aspects_ids:
                len_aspects_ids += len(aspect_ids)
        else:
            n_aspects = 0
        ...
        # Handle max sequence length
        total_len = len_ids + len_pair_ids + len_aspects_ids + (self.num_added_tokens_lcf(pair=pair, n_aspects=n_aspects) if add_special_tokens else 0)
        # print(len_ids)
        # print(len_pair_ids)
        # print(len_aspects_ids)
        # print(total_len)
        # print(n_aspects)
        
        if max_length and total_len > max_length:
            ids, pair_ids, aspects_ids, overflowing_tokens = self.truncate_sequences_lcf(
                ids,
                pair_ids=pair_ids,
                aspects_ids=aspects_ids,
                num_tokens_to_remove=total_len - max_length,
        ...
        # Handle special_tokens
        if add_special_tokens:
            sequence = self.build_inputs_with_special_tokens_lcf(ids, pair_ids, aspects_ids)
            token_type_ids = self.create_token_type_ids_from_sequences_lcf(ids, pair_ids, aspects_ids)
        else:
            if aspects_ids is None:
                if pair:
                ...
                aspects = []
                for aspect_ids in aspects_ids:
                    aspects.extend(aspect_ids)
                if pair:
                    sequence = ids + pair_ids + aspects
                    token_type_ids = [0] * len(ids) + [1] * len(pair_ids + aspects)
                else:
                    sequence = ids + aspects
                    token_type_ids = [0] * len(ids) + [1] * len(aspects)


        if return_special_tokens_mask:
            encoded_inputs["special_tokens_mask"] = self.get_special_tokens_mask_lcf(ids, pair_ids, aspects_ids)
    ...
    def truncate_sequences_lcf(
        self, ids, pair_ids=None, aspects_ids=None, num_tokens_to_remove=0, truncation_strategy="longest_first", stride=0
    ):
        """Truncates a sequence pair in place to the maximum length.
            truncation_strategy: string selected in the following options:
                - 'longest_first' (default) Iteratively reduce the inputs sequence until the input is under max_length
        ...
        if truncation_strategy == "longest_first":
            overflowing_tokens = []
            for _ in range(num_tokens_to_remove):
                if ((pair_ids is None or (len(ids) > len(pair_ids))) and len(ids) > 0):
                    overflowing_tokens = [ids[-1]] + overflowing_tokens
                    ids = ids[:-1]
                elif pair_ids is not None and len(ids) <= len(pair_ids):
                    pair_ids = pair_ids[:-1]
                else:
                    aspects_ids = ((aspects_ids[:-1] + [aspects_ids[-1][:-1]]) if len(aspects_ids[-1]) > 1 else aspects_ids[:-1])
        ...
        return (ids, pair_ids, aspects_ids, overflowing_tokens)
    ...
    def create_token_type_ids_from_sequences_lcf(self, token_ids_0, token_ids_1=None, aspects_ids=None):
        if aspects_ids is None:
            if token_ids_1 is None:
                return len(token_ids_0) * [0]
            return [0] * len(token_ids_0) + [1] * len(token_ids_1)
            ...
            for aspect_ids in aspects_ids:
                aspects.extend(aspect_ids)
            if token_ids_1 is None:
                return len(token_ids_0) * [0] + len(aspects) * [1]
            return len(token_ids_0) * [0] + len(token_ids_1 + aspects) * [1]
    ...
    def build_inputs_with_special_tokens_lcf(self, token_ids_0, token_ids_1=None, aspects_ids=None):
        """
        Build model inputs from a sequence or a pair of sequence for sequence classification tasks
        by concatenating and adding special tokens.
        A RoBERTa sequence has the following format:
        ...
        if aspects_ids is None:
            ...
            for aspect_ids in aspects_ids:
                aspects.extend(aspect_ids)
            if token_ids_1 is None:
                return token_ids_0 + aspects
            return token_ids_0 + token_ids_1 + aspects
    ...
    def get_special_tokens_mask_lcf(self, token_ids_0, token_ids_1=None, aspects_ids=None, already_has_special_tokens=False):
        """
        Retrieves sequence ids from a token list that has no special tokens added. This method is called when adding
        special tokens using the tokenizer ``prepare_for_model`` or ``encode_plus`` methods.
        ...
        if aspects_ids is None:
            ...
            aspects = []
            for aspect_ids in aspects_ids:
                aspects.extend([0] * len(aspect_ids))
            if token_ids_1 is None:
                return ([0] * len(token_ids_0)) + aspects
            return ([0] * len(token_ids_0)) + ([0] * len(token_ids_1)) + aspects
